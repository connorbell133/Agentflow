...
function createUserModelMapHelper(user_group_map: Record<string, { group_ids: string[] }>, model_group_map: Record<string, { group_ids: string[] }>): Array<{ user: string, models: string[] }> {
    ...
    const user_model_map = Object.entries(user_group_map).map(([user_id, user_data]) => {
        ...
        const accessible_models = Object.entries(model_group_map)
            .filter(([model_id, model_data]) => {
                ...
                return user_data.group_ids.some(group_id =>
                    model_data.group_ids.includes(group_id)
                )
            })
...
async function initiateSeed(seed: SeedClient, master_id: string): Promise<void> {
...
async function createOrganization(seed: SeedClient, org_id: string, master_id: string): Promise<void> {
...
async function createGroups(seed: SeedClient, org_id: string, master_id: string, admin_group_id: string, member_group_id: string, guest_group_id: string): Promise<void> {


    // Create groups for the organization
    await seed.groups((x) => x(1, {
        id: admin_group_id,
        role: 'admin',
...
async function createUsers(seed: SeedClient, org_id: string, master_id: string, member_group_id: string): Promise<string[]> {
    const users = []
    for (let i = 0; i < 10; i++) {
...
async function createOrgUsers(seed: SeedClient, org_id: string, users: string[], group_ids: string[], max_groups: number = 2): Promise<Record<string, { group_ids: string[] }>> {
...
async function createModels(seed: SeedClient, org_id: string, master_id: string): Promise<string[]> {
    ...
    const model_data = [
        {
            ...
            headers: {},
            body_config: {},
            message_format_config: { mapping: { role: { source: "role", target: "role", transform: "none" }, content: { source: "content", target: "content", transform: "none" } } },
            suggestion_prompts: []
        },
        {
            id: uuidv4(),
            ...
            message_format_config: { preset: "openai", mapping: { role: { source: "role", target: "role", transform: "none", roleMapping: [] }, content: { source: "content", target: "content", transform: "none" } }, attachments: { enabled: true, mapping: { image: { field: "image_data", format: "base64", wrapper: "data:{{mimeType}};base64,{{content}}" } }, bodyMapping: {}, acceptedTypes: ".png,.jpeg,.webp" }, customFields: [] },
        ...
        },
        {
            ...
            message_format_config: { preset: "openai", mapping: { role: { source: "role", target: "role", transform: "none", roleMapping: [{ to: "agent", from: "assistant" }] }, content: { source: "content", target: "content", transform: "none" } } },
            suggestion_prompts: []
        }
    ]

    for (const model of model_data) {
        await seed.models((x) => x(1, {
            id: model.id,
...
async function createModelMap(seed: SeedClient, org_id: string, master_id: string, model_ids: string[], group_ids: string[], max_groups: number = 2): Promise<Record<string, { group_ids: string[] }>> {
...
async function createConversations(seed: SeedClient, org_id: string, user_model_map: Array<{ user: string, models: string[] }>): Promise<string[]> {
    ...
    for (const { user: user_id, models: model_ids } of user_model_map) {
        for (const model_id of model_ids) {
            ...
            await seed.conversations((x) => x(1, {
                id: conversation_id,
                created_at: new Date(),
                user: user_id,
                model: model_id,
...
async function createMessages(seed: SeedClient, conversation_ids: string[]): Promise<void> {
    let totalMessages = 0
    for (const conversation_id of conversation_ids) {
        for (let i = 0; i < 5; i++) {
            const message_id = uuidv4()
            await seed.public_messages((x) => x(1, {
                id: message_id,
                created_at: new Date(),
