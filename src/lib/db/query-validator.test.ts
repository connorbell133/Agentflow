...
describe('Query Validator - SQL Injection Prevention', () => {
  ...
  describe('validateorg_id', () => {
    it('should accept valid UUID formats', () => {
      const validUuid = '123e4567-e89b-12d3-a456-426614174000';
      expect(validateorg_id(validUuid)).toBe(validUuid);
    });

    it('should reject invalid org IDs', () => {
      expect(() => validateorg_id(null)).toThrow('Invalid organization ID');
      expect(() => validateorg_id(undefined)).toThrow('Invalid organization ID');
      expect(() => validateorg_id('')).toThrow('Invalid organization ID');
      expect(() => validateorg_id('   ')).toThrow('Invalid organization ID');
    });

    it('should reject non-UUID formats', () => {
      expect(() => validateorg_id('not-a-uuid')).toThrow('Invalid organization ID format');
      expect(() => validateorg_id('123')).toThrow('Invalid organization ID format');
      expect(() => validateorg_id('g23e4567-e89b-12d3-a456-426614174000')).toThrow(
    ...
    it('should prevent SQL injection attempts', () => {
      ...
      expect(() => validateorg_id(sqlInjection)).toThrow('Invalid organization ID format');

      const unionInjection = "' UNION SELECT * FROM users --";
      expect(() => validateorg_id(unionInjection)).toThrow('Invalid organization ID format');

      const orInjection = "' OR '1'='1";
      expect(() => validateorg_id(orInjection)).toThrow('Invalid organization ID format');
    });

    it('should trim whitespace from valid UUIDs', () => {
      const uuid = '123e4567-e89b-12d3-a456-426614174000';
      expect(validateorg_id(`  ${uuid}  `)).toBe(uuid);
    });
  });
  ...
  describe('validateSortDirection', () => {
    ...
    it('should prevent SQL injection in sort direction', () => {
      expect(() => validateSortDirection('asc; DELETE FROM users;')).toThrow(
  ...
  describe('validateLimit', () => {
    ...
    it('should respect max limit', () => {
      ...
      expect(() => validateLimit(101, 50, 100)).toThrow(
        'Limit exceeds maximum allowed value of 100'
      );
    });
