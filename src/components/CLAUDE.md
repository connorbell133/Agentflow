...
See @shared/CLAUDE.md for shared, reusable components
...
2. **Shared Components** (`/shared`)
   ...
   - Previously named "common"

3. **Feature Components** (`/features`)
   - Business logic implementation
   - Feature-specific state
   - Composed from UI and Shared components
...
// With barrel files (cleaner)
import { ChatBox, ConversationList } from '@/components/features/chat';
import { EmptyState, LoadingSpinner } from '@/components/shared';

// Type imports (colocated with components)
import type { ChatBoxProps } from '@/components/features/chat/web/ChatBox/ChatBox.types';
```
...
```typescript
// ComponentName.types.ts (colocated)
