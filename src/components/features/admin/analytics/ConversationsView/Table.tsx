...
import { TableSkeleton } from '@/components/common/loading/SkeletonLoader';
  ...
  const renderRow = (
    ...
    return (
      <tr key={conversation.id}>
        <td className="px-6 py-4">{getUserById(conversation.user)?.email}</td>
        ...
        <td className="px-6 py-4">{conversation.title}</td>
        <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <ConversationPopup
            ...
            setActiveConversation={id =>
              setCurrentConversation(id ? conversations.data.find(c => c.id === id) || null : null)
            }
