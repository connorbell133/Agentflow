...
export default function Models({
    ...
    const handleDeleteConfirm = async () => {
        if (!deletingModel || confirmModelName !== (deletingModel.nice_name || deletingModel.model_id)) {
            return;
        }
    ...
    return (
        <>
            ...
            {isLoading && models.length === 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" data-testid="models-loading-grid">
                    {Array.from({ length: 6 }).map((_, index) => (
                        <SkeletonCard key={index} />
                    ))}
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" data-testid="models-grid">
                    {models.map((model) => (
                        <ModelCard
                            key={model.id}
                            id={model.id}
                ...
                <Dialog open={true} onOpenChange={handleDeleteCancel}>
                    <DialogContent className="max-w-sm" data-testid="delete-model-dialog">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                To delete {deletingModel.nice_name || deletingModel.model_id}, please type the AI connection name to confirm.
                        ...
                        <DialogFooter>
                            <Button variant="outline" onClick={handleDeleteCancel} data-testid="delete-model-cancel-button">
                                Cancel
                            </Button>
                            <Button
                                variant="destructive"
                                onClick={handleDeleteConfirm}
                                disabled={confirmModelName !== (deletingModel.nice_name || deletingModel.model_id) || isDeleting}
