...
export function transformMessages(
  ...
  return messages.map(msg => {
    ...
    Object.entries(config.mapping).forEach(([fieldName, mapping]) => {
      ...
      if (!mapping || !mapping.target || typeof mapping.target !== 'string' || mapping.target.trim() === '') {
 ...
 */
export function validatemessage_format_config(config: message_format_config): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
