...
import { addInvite, getInviteGroup, getOrgInvites, getOrgInvitesWithDisplayData, getUserInvites, removeInvite, getInviteOrg, getInviteInviter, acceptInviteAction } from '@/actions/organization/invites';
...
export const useInvites = () => {
    ...
    const fetchInvites = useCallback(async (user_email: string): Promise<{
        invites: Invite[],
        ...
        if (data) {
            ...
            const displayData = new Map<string, { orgName: string, groupName: string, inviterEmail: string }>();
    ...
    const fetchOrgInvites = useCallback(async (org_id: string): Promise<{
        invites: Invite[],
        displayData: Map<string, { orgName: string, groupName: string, inviterEmail: string }>
    }> => {
        try {
    ...
    const sendInvite = async (invitee: string, org_id: string, group_id: string, inviter: string): Promise<boolean> => {
    ...
    return { fetchInvites, acceptInvite, denyInvite, sendInvite, fetchOrgInvites, deleteInvite, isLoading };
};
