...
export async function getOrgUsers(org_id: string, options?: { page?: number; limit?: number }) {
  try {
    ...
    const userIds = options ? sortedUserIds.slice(offset, offset + limit) : sortedUserIds;
    ...
    // Avatar URLs are stored directly in the profiles table
    // No need for external enrichment
