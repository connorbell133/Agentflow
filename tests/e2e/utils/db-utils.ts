 ...
 * @returns The user ID (Clerk user ID stored in profiles.id)
...
export async function createInviteInDatabase(
    ...
    if (error || !invite) {
        throw new Error(
            ...
            `Error: ${error?.message || 'No invite returned'}`
        );
    }
...
export async function deleteInviteFromDatabase(inviteId: string): Promise<void> {
    ...
    if (error) {
        throw new Error(
            `Failed to delete invite ${inviteId} from database. ` +
            `Error: ${error.message}`
        );
    }
}
...
export async function deleteInvitesByEmail(inviteeEmail: string): Promise<void> {
    ...
    const { error } = await supabase
        .from('invites')
        .delete()
        .eq('invitee', inviteeEmail);

    if (error) {
        throw new Error(
            `Failed to delete invites for ${inviteeEmail}. ` +
            `Error: ${error.message}`
        );
    }
}
...
export async function deleteGroupFromDatabase(orgId: string, groupName: string): Promise<void> {
    ...
    if (error) {
        throw new Error(
            `Failed to delete group "${groupName}" from org ${orgId}. ` +
            `Error: ${error.message}`
        );
    }
}
...
export async function addUserToGroupInDatabase(
    ...
    if (checkError && checkError.code !== 'PGRST116') {
        throw new Error(
            `Failed to check group membership: ${checkError.message}`
        );
    }
    ...
    if (error) {
        throw new Error(
            `Failed to add user ${userEmail} to group "${groupName}". ` +
            `Error: ${error.message}`
        );
    }
}
...
export async function removeUserFromGroupInDatabase(
    ...
    if (error) {
        throw new Error(
            `Failed to remove user ${userEmail} from group "${groupName}". ` +
            `Error: ${error.message}`
        );
    }
}
...
export async function addModelToGroupInDatabase(
    ...
    if (checkError && checkError.code !== 'PGRST116') {
        throw new Error(
            `Failed to check model-group mapping: ${checkError.message}`
        );
    }
    ...
    if (error) {
        throw new Error(
            `Failed to add model "${modelName}" to group "${groupName}". ` +
            `Error: ${error.message}`
        );
    }
}
 ...
 * Note: This creates a profile only - the user will NOT be able to sign in via Clerk.
 ...
 */
export async function addUserToOrgInDatabase(
    orgId: string,
    userId: string
): Promise<void> {
    const supabase = await createSupabaseServerClient();
    ...
    if (checkError && checkError.code !== 'PGRST116') {
        throw new Error(
            `Failed to check org membership: ${checkError.message}`
        );
    }
    ...
    if (error) {
        throw new Error(
            `Failed to add user ${userId} to org ${orgId}. ` +
            `Error: ${error.message}`
        );
    }
}
...
export async function createTestUserInOrg(
    ...
    // Generate a unique user ID (using email-based UUID for consistency)
 ...
 */
export async function removeUserFromOrgInDatabase(
    orgId: string,
    userId: string
): Promise<void> {
    const supabase = await createSupabaseServerClient();
    ...
    if (error) {
        throw new Error(
            `Failed to remove user ${userId} from org ${orgId}. ` +
            `Error: ${error.message}`
        );
    }
}
...
export async function deleteTestUserProfile(userId: string): Promise<void> {
    ...
    if (groupMapError) {
        throw new Error(
            `Failed to remove user ${userId} from groups. ` +
            `Error: ${groupMapError.message}`
        );
    }
    ...
    if (orgMapError) {
        throw new Error(
            `Failed to remove user ${userId} from orgs. ` +
            `Error: ${orgMapError.message}`
        );
    }
    ...
    if (error) {
        throw new Error(
            `Failed to delete test user profile ${userId}. ` +
            `Error: ${error.message}`
        );
    }
}
...
export async function deleteTestModel(modelId: string, orgId: string): Promise<void> {
    ...
    if (modelMapError) {
        throw new Error(
            `Failed to remove model ${modelId} from groups. ` +
            `Error: ${modelMapError.message}`
        );
    }
    ...
    if (error) {
        throw new Error(
            `Failed to delete test model ${modelId}. ` +
            `Error: ${error.message}`
        );
    }
}
