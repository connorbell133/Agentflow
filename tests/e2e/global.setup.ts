...
setup('authenticate admin user and create org', async () => {
  ...
  try {
    const cleanupResult = await cleanupAllTestUsers(
      'test+%@test.com', // Pattern matches all test emails generated by generateTestEmail()
    ...
    if (cleanupResult.success) {
      ...
      console.log(`✅ Cleaned up ${total} items from previous runs`);
    } else {
      console.warn('⚠️  Cleanup had some errors:', cleanupResult.errors);
    }
  } catch (error) {
    console.warn('⚠️  Cleanup failed, but continuing with setup:', (error as Error).message);
  }
  ...
  if (!userId) {
    console.log('📝 Creating new admin user with organization via direct DB calls...');
    ...
    console.log('✅ Created admin user, organization, and saved auth state');
    return;
  }

  // Scenario 2: User exists but no org - create org
  if (!orgId) {
    console.log('🏢 Creating organization for existing user via direct DB calls...');
  ...
  console.log('🔑 Signing in existing user and saving auth state...');
