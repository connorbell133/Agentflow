...
teardown('cleanup all test data', async () => {
  ...
  console.log('\n🧹 Running global teardown - cleaning up all test data...');

  try {
    // Clean up all test users except the admin
    // The pattern 'test.%@example.com' matches all test emails generated by factories
    ...
    if (!result.success) {
      console.error('❌ Teardown had errors:', result.errors);
    } else {
      console.log('\n📊 Teardown Summary:');
